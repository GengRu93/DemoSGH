...
if __name__ == "__main__":
    ...
    for sim in range(data_test.shape[0]):
        ...
        test_loss_symnetLA=L2_loss(dx,label_test[sim])

        test_loss.append(test_loss_symnetLA.data.numpy())
    ...
    print('test_loss_symnetLA:',np.mean(np.abs(test_loss)))
    print('test_loss_symnetLA_std:', np.std(test_loss))
    ...
    for i in range(200):
        ...
        im_particle.append(plt.plot(pos[0:N], grouth_sol[t_index][i, 0:N], 'ko', markersize=10) + plt.plot(pos[0:N],
                                                                                                           pred_sol[
                                                                                                               t_index][
                                                                                                          i, 0:N],
                                                                                                           'ro',
    ...
    plt.title('FPU-KG LA-SympNet prediction')

    im_ani.save('MixFPU_LA-SympNet.gif', writer='pillow', fps=60)
    plt.show()