...
def integrate_model(model_step1,t_span,edge_index, y0,t_eval, **kwargs):
    def fun(t, np_x):
...
if __name__ == "__main__":
    ...
    for sim in range(data_test.shape[0]):
        ...
        dx,dvt = model_step1(data_test[sim].to(device), edge_index.to(device))
        test_loss_sghn=L2_loss(dvt,label_test[sim])

        test_loss.append(test_loss_sghn.data.numpy())


        sol = solver(func, [0, time], data_testnp[sim][0,:], t_eval=t_eval, **kwargs)
        grouth_sol[sim] = sol['y'].T

        hnn_path = integrate_model(model_step1, [0, tend], edge_index, data_testnp[sim][0,:],t_eval, **kwargs)
        hnn_x = hnn_path['y'].T
        pred_sol[sim] = hnn_x


    print('test_loss_sghn:',np.mean(np.abs(test_loss)))
    print('test_loss_sghn_std:', np.std(test_loss))
    ...
    print('enegry_loss_sghn:', np.mean(np.abs(loss_enery)))
    print('enegry_loss_sghn_std:', np.std(loss_enery))
    ...
    for i in range(index.shape[0]):
        ...
        im_particle.append(
            plt.plot(pos[0:N], grouth_sol[t_index][index[i], 0:N], 'ko', markersize=10) + plt.plot(pos[0:N],
                                                                                                   pred_sol[
                                                                                                       t_index][
                                                                                                   index[i], 0:N],
                                                                                                   'ro',
    ...
    plt.title('FPU-KG SGHN prediction')

    im_ani.save('MixFPU_sghn.gif', writer='pillow', fps=60)
    plt.show()
