...
def train(args):
    ...
    for sim in range(1):
        ...
        model_step1 = HNN(args, device).to(device)
        ...
        for step in range(args.total_steps1 + 1):
            ...
            for batch in range(no_batches):
                ...
                dxdt_hat = model_step1(x[ixs].to(device))
            ...
            acc_val = L2_loss(dxdt_val.to(device),
                              dxdt_hat_val)
            loss_values_val.append(acc_val.item())


            os.makedirs(args.save_dir_hnn200) if not os.path.exists(args.save_dir_hnn200) else None
            label1 = str(sim) + '-hnn200.'

            path1 = '{}/{}{}{}.tar'.format(args.save_dir_hnn200, args.name, label1, step)
            ...
            files = glob.glob(args.save_dir_hnn200 + '/*.tar')
            # print(files)
            ...
            for file in files:
                ...
                if labell == 'Train' + str(sim) + '-hnn200':
        ...
        name_save_loss = './loss/HNN200_' + str(sim) + '_loss'
