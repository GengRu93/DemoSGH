    ...
    for sim in range(1):
        ...
        for step in range(args.total_steps1 + 1):
            ...
            for batch in range(no_batches):
                ...
                dxdt_hat, dvt_hat = model_step1(x[ixs].to(device))

                loss = L2_loss(dxdt[ixs].to(device),
                               dvt_hat)
            ...
            model_step1.eval()
            dxdt_hat_val, dvt_hat_val = model_step1(x_val.to(device))
            acc_val = L2_loss(dxdt_val.to(device), dvt_hat_val)

            loss_values_val.append(acc_val.item())


            os.makedirs(args.save_dir_hnnp2) if not os.path.exists(args.save_dir_hnnp2) else None
            label1 = str(sim)+'-hnnp2.'

            path1 = '{}/{}{}{}.tar'.format(args.save_dir_hnnp2, args.name, label1, step)
            ...
            files = glob.glob(args.save_dir_hnnp2+'/*.tar')
        ...
        files = glob.glob(args.save_dir_hnnp2+'/*.tar')
        name_save_loss='./loss/HNNp22_' + str(sim) + '_loss'
