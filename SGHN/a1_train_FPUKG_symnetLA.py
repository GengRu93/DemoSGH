...
def train(args):
    ...
    xlabel_train = x[1:, :]
    X_val = x_val[:-1, :]
    print('X_train.shape',X_train.shape) #torch.Size([9999, 64])

    xlabel_val = x_val[1:, :]

    no_batches = int(X_train.shape[0] / args.batch_size)
    ...
    for sim in range(1):
        ...
        model_step1 = LASympNet(args.n_particle*2, args.symp_LAlayers, args.symp_LAsublayers, args.symp_activation).to(device)


        print("Num. of params: {:d}".format(utils.get_parameters_count(model_step1)))

        parmas = list(model_step1.parameters())
        optim1 = torch.optim.Adam(parmas, args.learn_rate1)
        optim2 = torch.optim.Adam(parmas, args.learn_rate2)
        ...
        for step in range(args.total_steps1 + 1):
            ...
            for batch in range(no_batches):
                ixs = torch.randperm(X_train.shape[0])[:args.batch_size]

                dxdt_hat = model_step1(X_train[ixs].to(device))

                loss = L2_loss(xlabel_train[ixs].to(device), dxdt_hat)
            ...
            model_step1.eval()
            dxdt_hat_val = model_step1(X_val.to(device))
            acc_val = L2_loss(xlabel_val.to(device),
                              dxdt_hat_val)  # +L2_loss(dxdt_hat[:,31],fix.to(device))+L2_loss(dxdt_hat[:,0],fix.to(device))+L2_loss(dxdt_hat[:,32],fix.to(device))+L2_loss(dxdt_hat[:,63],fix.to(device))
            loss_values_val.append(acc_val.item())


            os.makedirs(args.save_dir_symnetLA) if not os.path.exists(args.save_dir_symnetLA) else None
            label1 = str(sim)+'-symnetLA.'

            path1 = '{}/{}{}{}.tar'.format(args.save_dir_symnetLA, args.name, label1, step)
            ...
            files = glob.glob(args.save_dir_symnetLA+'/*.tar')
            ...
            for file in files:
                ...
                if labell == 'Train'+str(sim)+'-symnetLA':
            ...
            if args.verbose and step % args.print_every == 0:
                print("sim {},step {}, train_loss {:.4e},acc_loss {:.4e}| time: {:>7.12f}".format(sim,step, train_loss_epoch_even / no_batches,acc_val,(time.time() - tt0)))
        files = glob.glob(args.save_dir_symnetLA+'/*.tar')
        name_save_loss='./loss/symnetLA_' + str(sim) + '_loss'
