...
if __name__ == "__main__":
    ...
    label1 = str(0) + '-hnn200.'
    best_step = best
    path1 = '{}/{}{}{}.tar'.format(args.save_dir_hnn200, args.name, label1, best_step)
    ...
    for sim in range(data_test.shape[0]):
        ...
        test_loss_hnn=L2_loss(dx,label_test[sim])

        test_loss.append(test_loss_hnn.data.numpy())
    ...
    print('test_loss_hnn:',np.mean(np.abs(test_loss)))
    print('test_loss_hnn_std:', np.std(test_loss))
    ...
    plt.title('FPU-KG HNN prediction')

    im_ani.save('MixFPU_hnn.gif', writer='pillow', fps=60)
    plt.show()
